...
class LocalMedSAM:
    ...
    def load_model(self):
        try:
            logger.info(f"Loading MedSAM model from {self.model_path} onto {self.device}...")
        ...
        except Exception as e: logger.error(f"Failed to load MedSAM model: {e}")
...
class LocalROIAnalyzer:
    ...
    def load_model(self):
        try:
            logger.info(f"Loading MedSigLIP model onto {self.device}...")
            model_name = "google/medsiglip-448"
            self.model = AutoModel.from_pretrained(model_name).to(self.device)
            self.processor = AutoProcessor.from_pretrained(model_name)
            self.is_ready = True
            logger.info("Local ROI Analyzer is ready.")
        except Exception as e: logger.error(f"Failed to load MedSigLIP model: {e}")
...
def analyze_roi_anatomical_overlap(roi_region: Union[RectangularROI, CircularROI], anatomical_map: Dict) -> Dict:
    ...
    for structure in anatomical_map["structures"]:
        ...
        struct_bbox = RectangularROI(
            Point(bbox["x_min"] * 512, bbox["y_min"] * 512),
            Point(bbox["x_max"] * 512, bbox["y_max"] * 512)
        )
...
def find_overlapping_structures(canvas_x: float, canvas_y: float, anatomical_map: Dict) -> List[Dict]:
    ...
    """
    # Convert canvas coordinates (0-512) to normalized coordinates (0-1)
    norm_x = canvas_x / 512.0
    norm_y = canvas_y / 512.0
...
class RectangleROIRequest(BaseModel):
    top_left_x: float = Field(..., ge=0, le=512, description="Top-left X coordinate")
    top_left_y: float = Field(..., ge=0, le=512, description="Top-left Y coordinate") 
    bottom_right_x: float = Field(..., ge=0, le=512, description="Bottom-right X coordinate")
    bottom_right_y: float = Field(..., ge=0, le=512, description="Bottom-right Y coordinate")

class CircleROIRequest(BaseModel):
    center_x: float = Field(..., ge=0, le=512, description="Center X coordinate")
    center_y: float = Field(..., ge=0, le=512, description="Center Y coordinate")
    radius: float = Field(..., gt=0, le=256, description="Radius in pixels")
...
async def analyze_rectangle_roi_endpoint(
    ...
    top_left_x: float = Query(..., ge=0, le=512, description="Top-left X coordinate"),
    top_left_y: float = Query(..., ge=0, le=512, description="Top-left Y coordinate"),
    bottom_right_x: float = Query(..., ge=0, le=512, description="Bottom-right X coordinate"),
    bottom_right_y: float = Query(..., ge=0, le=512, description="Bottom-right Y coordinate")
):
...
async def analyze_circle_roi_endpoint(
    slice_index: int = Query(..., description="Slice index to analyze"),
    center_x: float = Query(..., ge=0, le=512, description="Center X coordinate"),
    center_y: float = Query(..., ge=0, le=512, description="Center Y coordinate"),
    radius: float = Query(..., gt=0, le=256, description="Radius in pixels")
):
...
async def _analyze_roi_shared(slice_index: int, roi_region, roi_description: str, roi_type: str, roi_request_dict: dict):
    ...
    vol_x, vol_y = int(roi_center.x * (w / 512)), int(roi_center.y * (h / 512))
...
async def preview_whole_image_analysis(
    ...
    try:
        ...
        if np.ptp(slice_2d) > 0:
            normalized = (slice_2d - np.min(slice_2d)) / np.ptp(slice_2d) * 255
        else:
...
async def analyze_roi_endpoint(slice_index: int = Query(...), canvas_x: float = Query(...), canvas_y: float = Query(...)):
    ...
    try:
        ...
        h, w = full_slice_np.shape
        vol_x, vol_y = int(canvas_x * (w / 512)), int(canvas_y * (h / 512))
        mask = local_medsam.get_mask(full_slice_np, (vol_x, vol_y))
